...
var GetTickets = `SELECT * FROM flight_ticket`

var GetPassengersByTicketNumber = `
SELECT p.passenger_id, p.last_name, p.first_name, p.middle_name
FROM passenger_ticket pt 
JOIN passenger p ON pt.passenger_id = p.passenger_id
JOIN flight_ticket ft ON pt.ticket_id = ft.ticket_id
WHERE ft.order_number = $1;
`
...
FROM 
    flight_ticket ft 
JOIN 
    passenger_ticket pt ON ft.ticket_id = pt.ticket_id
JOIN
	passenger p ON pt.passenger_id = p.passenger_id
LEFT JOIN 
...
`
var DeleteTicketPassenger = `
DELETE FROM passenger_ticket WHERE ticket_id = $1;
`
var DeletePassenger = `
